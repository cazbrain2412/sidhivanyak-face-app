...
export default function AdminSupervisorsPage() {
  ...
  setLoading(true);
  try {
    // ✅ ZONE-ADMIN FILTERED SUPERVISORS
    const s = await fetch("/api/zone-admin/supervisors/list")
      .then((r) => r.json());
    setList(s.supervisors || []);

    // zones (used only for labels / dropdown)
    const z = await fetch("/api/zones/list")
      .then((r) => r.json());
    setZones(z.zones || []);

    // divisions (already zone-filtered later if needed)
  ...
  async function editSupervisor(code) {
    ...
    const payload = { code, name: newName };
